...
/**
 * column for CopyCoprocessor rule
 * @author lingbo.zm
 *
 */
public class Column {
    ...
    public Column(String column) {
		...
		if (deliIdx > 0) {
		    ...
		    int qualifierRegIdx = qualifier.indexOf(CopyCoprocessorConstants.JSON_WILDCHAR);
			if (qualifierRegIdx >= 0) {
				qualifierPattern = Pattern.compile(qualifier.replaceAll(CopyCoprocessorConstants.JSON_WILDCHAR_REGEX, CopyCoprocessorConstants.JSON_WILDCHAR_REPLACE));
			}
		} else {
			family = column;
		}
	...
	private boolean matchesQualifier(String qualifier) {
		if (this.qualifier == null || this.qualifier.isEmpty()) {
			// column qualifier not specified for copy rule
			return true;
		}
