...
public class TestAsyncGetWithPartialResult {
  ...
  @Test
  public void testAsyncGetLargeRow() throws IOException, InterruptedException, ExecutionException {
    TEST_UTIL.getConfiguration().set(RpcClientFactory.CUSTOM_RPC_CLIENT_IMPL_CONF_KEY,
	      AsyncRpcClient.class.getName());
	moveRegion(TABLE,1);
    
    Get get = new Get(ROWS[0]);
    AsyncFuture<Result> f = ((HTable)TABLE).asyncGet(get);
	Result rs1 = f.get();
	assertEquals(NUM_COLS, rs1.size());
	
	moveRegion(TABLE,2);
	((HTable)TABLE).asyncGet(get);
	Result rs2 = f.get();
	assertEquals(NUM_COLS, rs2.size());
	compareResults(rs1, rs2, "the two result should be equal!");
	
	rs2 = ((HTable)TABLE).get(get);
	assertEquals(NUM_COLS, rs2.size());
	compareResults(rs1, rs2, "the two result should be equal!");
  }
}
