...
public class TestStoreScanner extends TestCase {
  ...
  public void testMemStoreRef() throws IOException {
    // build the scan file:
    long timestamp = System.currentTimeMillis();
    KeyValue[] kvs =
        new KeyValue[] {
            KeyValueTestUtil.create("R1", "cf", "a", timestamp, KeyValue.Type.Put, "dont-care"),
            KeyValueTestUtil.create("R2", "cf", "a", timestamp, KeyValue.Type.Put, "dont-care"),
            KeyValueTestUtil.create("R3", "cf", "a", timestamp, KeyValue.Type.Put, "dont-care") };
    List<KeyValueScanner> scanners = scanFixture(kvs);

    DefaultMemStore memstore = new DefaultMemStore();
    MemstoreSize memstoreSize = new MemstoreSize();
    memstore.add(KeyValueTestUtil
        .create("R4", "cf", "a", timestamp, KeyValue.Type.Put, "dont-care"), memstoreSize);
    memstore.add(KeyValueTestUtil
        .create("R5", "cf", "a", timestamp, KeyValue.Type.Put, "dont-care"), memstoreSize);
    memstore.add(KeyValueTestUtil
        .create("R6", "cf", "a", timestamp, KeyValue.Type.Put, "dont-care"), memstoreSize);
    List<KeyValueScanner> memStoreScanners = memstore.getScanners(Long.MAX_VALUE);
    ...
    // Do not touch MemStoreScanner
    MemStoreScanner memStoreScanner = (MemStoreScanner) memStoreScanners.get(0);
    assertFalse(memStoreScanner.isReferenced());
    //assertTrue(memStoreScanner.allocatorAtCreation != null);
    scan.updateReaders();
    //assertTrue(memStoreScanner.allocatorAtCreation == null);

    // Touch MemStoreScanner
    scanners = scanFixture(kvs);
    memStoreScanners = memstore.getScanners(Long.MAX_VALUE);
    scanners.addAll(memStoreScanners);
    ...
    memStoreScanner = (MemStoreScanner) memStoreScanners.get(0);
    assertFalse(memStoreScanner.isReferenced());
    results.clear();
    scan.next(results);
    assertEquals(1, results.size());
    assertTrue(memStoreScanner.isReferenced());
