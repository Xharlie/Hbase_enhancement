...
public class TestWALObserver {
  ...
  private void verifyWritesSeen(final WAL log, final SampleRegionWALObserver cp,
    ...
    long txid = log.append(hri, new HLogKey(hri.getEncodedNameAsBytes(), hri.getTable(), now,
        new MultiVersionConsistencyControl(), scopes),
      edit, true);
    log.sync(txid);
  ...
  private void addWALEdits(final TableName tableName, final HRegionInfo hri, final byte[] rowName,
      ...
      final NavigableMap<byte[], Integer> scopes, final MultiVersionConsistencyControl mvcc)
          throws IOException {
