...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner
      ...
      if (!isParallelSeek) {
        ...
        for (KeyValueScanner scanner : scanners) {
          ...
          if (totalScannersSoughtBytes >= maxRowSize) {
            LOG.warn("Big row: " + c.toString() + ", totalBytesRead: " + totalScannersSoughtBytes);
            if (matcher.isUserScan()) {
      ...
      switch(qcode) {
        ...
        case INCLUDE_AND_SEEK_NEXT_COL:
          ...
          if (this.countPerRow > storeOffset) {
            outResult.add(cell);
            KeyValueScanner curScanner = heap.getCurrent();
            if (curScanner instanceof MemStoreScanner) {
              ((MemStoreScanner) curScanner).setReferenced();
            }
            ...
            if (totalBytesRead > maxRowSize) {
              LOG.warn("Big row: " + cell.toString() + ", totalBytesRead: " + totalBytesRead);
              if (matcher.isUserScan()) {
                throw new RowTooBigException("Max row size allowed: " + maxRowSize
  ...
  public void updateReaders() throws IOException {
    ...
    // Close unreferenced MemStore scanners
    for (MemStoreScanner scanner : heap.getMemstoreScanners()) {
      if (!scanner.isReferenced()) {
        scanner.close();
      }
    }
