...
public class CompoundBloomFilter extends CompoundBloomFilterBase
  ...
  public CompoundBloomFilter(DataInput meta, HFile.Reader reader)
    ...
    String comparatorClassNameString = Bytes.toString(comparatorClassName);
    if (comparatorClassNameString.equals(KeyValue.RAW_COMPARATOR.getLegacyKeyComparatorName())
      ||comparatorClassNameString.equals(KeyValue.RAW_COMPARATOR.getClass().getName())) {
      comparator = null;
    }else {
      comparator = FixedFileTrailer.createComparator(comparatorClassNameString);
    }
