...
public class HFileReaderImpl implements HFile.Reader, Configurable {
  ...
  private final MetricsHFileSource hfileMetrics;
  ...
  protected static class HFileScannerImpl implements HFileScanner {
    ...
    // From non encoded HFiles, we always read back KeyValue or its derives. (Note : When HFile
          ...
          if (cachedBlock != null) {
            ...
            if (cachedBlock.getBlockType().isData()) {
              if (updateCacheMetrics) {
                HFile.DATABLOCK_READ_COUNT.incrementAndGet();
              }
