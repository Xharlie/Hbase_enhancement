...
public class MemStoreScanner extends NonLazyKeyValueScanner {
  ...
  private final AtomicBoolean referenced = new AtomicBoolean(false);
   ...
   */
  @Override
  public synchronized boolean shouldUseScanner(Scan scan, SortedSet<byte[]> columns, long oldestUnexpiredTS) {
    ...
    for (KeyValueScanner sc : scanners) {
      if (sc.shouldUseScanner(scan, columns, oldestUnexpiredTS)) {
        return true;
      }
    }
    return false;
