...
public class CompoundBloomFilterWriter extends CompoundBloomFilterBase
  ...
  private class MetaWriter implements Writable {
    ...
    public void write(DataOutput out) throws IOException {
      ...
      if (comparator != null) {
        if (!comparator.getClass().equals(CellComparator.COMPARATOR.getClass())) {
          throw new IOException("comparator should be of type CellComparator!");
        }
        //bloom filter comparator should be compatible with hfile v2
        comparatorName = KeyValue.COMPARATOR.getLegacyKeyComparatorName();
        Bytes.writeByteArray(out, Bytes.toBytes(comparatorName));
      } else {
        //bloom filter comparator should be compatible with hfile v2
        comparatorName = KeyValue.RAW_COMPARATOR.getLegacyKeyComparatorName();
        Bytes.writeByteArray(out, Bytes.toBytes(comparatorName));
      }
