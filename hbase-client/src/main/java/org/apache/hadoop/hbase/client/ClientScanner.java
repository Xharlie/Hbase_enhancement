...
public class ClientScanner extends AbstractClientScanner {
          ...
          if (timeout < System.currentTimeMillis()) {
            ...
            ScannerTimeoutException ex =
                new ScannerTimeoutException(elapsed + "ms passed since the last invocation, "
                    + "timeout is currently set to " + scannerTimeout + "; Current scan: "
                    + getScan().toString() + " on table: " + getTable().getNameAsString());
            ex.initCause(e);
            throw ex;
          }
        } else {
  ...
  private int compare(Cell a, Cell b) {
    KeyValue.KVComparator comparator = currentRegion != null && currentRegion.isMetaRegion() ?
