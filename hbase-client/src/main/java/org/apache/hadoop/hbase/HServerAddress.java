 ...
 * HServerAddress hosts a {@link InetSocketAddress} and makes it {@link WritableComparable}.
 * Resolves on construction AND on deserialization -- since we're internally creating an
 * InetSocketAddress -- so could end up with different results if the two ends of serialization have
 * different resolvers. Be careful where you use it. Should only be used when you need to pass an
 * InetSocketAddress across an RPC. Even then its a bad idea because of the above resolve issue.
 * @deprecated Use {@link InetSocketAddress} or {@link ServerName} or a hostname String and port.
 */
...
public class HServerAddress implements WritableComparable<HServerAddress> {
   ...
   *         InetSocketAddress#getAddress()#getHostAddress() -- or null if cannot resolve
   */
  ...
  private static String getBindAddressInternal(InetSocketAddress address) {
    ...
    if (addr != null) {
      ...
      LogFactory.getLog(HServerAddress.class).error(
        "Could not resolve the" + " DNS name of " + address.getHostName());
      return null;
    }
  }
  ...
  private void checkBindAddressCanBeResolved() {
    if (getBindAddress() == null) {
      throw new IllegalArgumentException("Could not resolve the" + " DNS name of "
