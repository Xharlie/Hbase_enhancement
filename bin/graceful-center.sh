...
function usage {
  echo "Usage: $0 [--config <conf-dir>] [--restart] [--start] [--stop] <hostname>"
...
. "$bin"/hbase-config.sh
export log4j.logger.org.apache.hadoop.hbase=INFO,console
# Get arguments
start=
restart=
stop=
...
hostname=$1
echo "grace operation regionserver is: $hostname"
echo "sleep 10s ..."
sleep 10
